...
// change stdin to non blocking
	...
	jni_sys_error(string); // Vladimir JNI
	//_exit (1);
...
void *Sys_GetGameAPI (void *parms)
{
	...
	char	*path;
#ifdef __i386__
	const char *gamename = "libgamei386.so";
#elif defined __alpha__
	const char *gamename = "libgameaxp.so";
...
#error Unknown arch
	...
	// Vladimir getcwd(curpath, sizeof(curpath));
	// Load quake2.conf from gamedir
	char so_file[64]; 
	char	fn[MAX_OSPATH];
	FILE *fp;
	...
	while (1)
	{
		...
		sprintf (name, "%s/%s", curpath, gamename); // Vladimir

		Com_DPrintf("Sys_GetGameAPI DLOPEN(%s)\n", name);
