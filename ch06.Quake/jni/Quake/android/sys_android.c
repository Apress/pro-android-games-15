	...
	for (p = (unsigned char *)text; *p; p++) {
		*p &= 0x7f;
		if ((*p > 128 || *p < 32) && *p != 10 && *p != 13 && *p != 9)
			...
			putchar(*p); // Vladimir putc(*p, stdout);
	}
#endif	
}
...
void Sys_Quit (void)
{
	...
	//  Vladimir   	fcntl (0, F_SETFL, fcntl (0, F_GETFL, 0) & ~FNDELAY);
	// Vladimir fflush(stdout);
/*	
#ifdef ANDROID
	jni_sys_quit();
#else	*/
...
//    fcntl (0, F_SETFL, fcntl (0, F_GETFL, 0) & ~FNDELAY); Vladimir
...
int Sys_FileOpenRead (char *path, int *handle)
{
	...
	*handle = h;
	...
	if (h == -1)
		return -1;
	/* Vladimir Crashes on android
	if (fstat (h,&fileinfo) == -1)
		Sys_Error ("Error fstating %s", path);
	*/

	int len = I_Filelength(h);
	//printf("sys_linux.c::Sys_FileOpenRead %s handle:%d size:%d\n", path, h, len);

	return len; //fileinfo.st_size;
}
...
//	Sys_Warn("floating point exception\n");
	// Vladimir signal(SIGFPE, floating_point_exception_handler);
	sysv_signal(SIGFPE, floating_point_exception_handler); 
}
...
char *Sys_ConsoleInput(void)
{
	...
	if (cls.state == ca_dedicated) {
		// Vladimir FD_ZERO(&fdset);
		// Vladimir FD_SET(0, &fdset); // stdin
		timeout.tv_sec = 0;
		timeout.tv_usec = 0;
		if (select (1, &fdset, NULL, NULL, &timeout) == -1 ) { // Vladimir || !FD_ISSET(0, &fdset))
			return NULL;
		}
...
{
	double		time, newtime;
	static double oldtime; // Vladimir (made this static)
	//quakeparms_t parms;
	/* Vladimir
	extern int vcrFile;
...
// find time spent rendering last frame
	...
	{   // play vcrfiles at max speed
		...
		{
			usleep(1);
			return; //continue;       // not time to run a server only tic yet
		}
...
//	parms.cachedir = cachedir;
	...
	//fcntl(0, F_SETFL, fcntl (0, F_GETFL, 0) ); // Vladimir| FNDELAY
...
{
	printf("Sys_MakeCodeWriteable start addr:%ld len:%ld not implemented.\n", startaddr, length);
/* Vladimir
	int r;
	unsigned long addr;
