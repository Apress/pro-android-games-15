...
void Key_Console (int key)
{
	...
	if (key == K_PGUP || key==K_MWHEELUP)
	{
		if (keydown[K_CTRL])
		{
			// Only page about half console height
			con_backscroll += (con_vislines>>3) / 2;
		}
		else
		{
			// Only one line in low resolutions
			con_backscroll += (con_vislines>>3) < 30 ? 1 : 2;
		}
	...
	if (key == K_HOME)
	{
		if (keydown[K_CTRL])
		{
			...
			if (i < con_current - (con_vislines>>3) + 3)
				con_backscroll = con_current - i - (con_vislines>>3) + 3;
		}
		else
		{
			// Only page about half console height
