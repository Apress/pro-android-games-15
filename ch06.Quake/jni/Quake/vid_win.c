...
// z, and surface buffers
	if ((host_parms.memsize - tbuffersize + SURFCACHE_SIZE_AT_320X200 +
		 0x10000 * 3) < minimum_memory)
	{
		Con_SafePrintf ("VID_AllocBuffers: Not enough memory for video mode\n");
		return false;		// not enough memory for mode
	}
	...
	d_pzsize = tbuffersize;

	if (!d_pzbuffer)
		Sys_Error ("VID_AllocBuffers: not enough memory (%d) for video buffer", tbuffersize);
...
int VID_Suspend (MGLDC *dc,m_int flags)
{
	...
	Sys_Error ("VID_Suspend: called with invalid flags (%x)", (unsigned)flags);
...
// to try to use WinDirect modes even on NT
	...
	// The above line seems to cause unhandled FPU exceptions later on some
	// platforms (at least on Win95); reset FPU exception logic, kludge
	if (!WinNT)
		_fpreset();
...
// fix the leftover Alt from any Alt-Tab or the like that switched us away
	...
	// Prevent unwanted printouts
	if (!msg_suppress_1 && con_x == 0)
...
void VID_LockBuffer (void)
{
	...
	if (r_dowarp)
	{
		...
		d_viewsize = WARP_WIDTH * WARP_HEIGHT;
	}
	else
	{
		d_viewbuffer = (void *)(byte *)vid.buffer;
		d_viewsize = vid.rowbytes * vid.height; // This is uncelar ...
	}
