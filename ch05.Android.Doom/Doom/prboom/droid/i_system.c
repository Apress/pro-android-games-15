...
extern void jni_sys_error(const char * text);
...
//  if non-existant
static const char prboom_dir[] = {"/studio3d/wad"}; // Mead rem extra slash 8/21/03
  ...
  if (!base)        // cache multiple requests
    {
	  ...
	  //jni_printf("I_DoomExeDir: Created dir %s\n", base);
    }
  return base;
}
...
char* I_FindFile(const char* wfname, const char* ext)
{
  ...
  if ( ! wfname ) {
	...
	jni_sys_error("I_FindFile BUG: File name cannot be NULL!\n");
	//exit (1);
  }
  if ( ! ext ) {
	//printf("I_FindFile BUG: File extension cannot be NULL!\n");
	jni_sys_error("I_FindFile BUG: File extension cannot be NULL!\n");
	//exit (1);
  }
